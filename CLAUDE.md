...
- **Known quirk — geopath on the pattern endpoint**: `/pattern/run/{run_ref}/route_type/{n}?include_geopath=true` returns `geopath: null` in practice (verified live for trains and buses, even with `expand=All`), and its `stops` sideload omits lat/lng. The reliable geopath source is `/stops/route/{route_id}/route_type/{n}?include_geopath=true`, which returns the polyline (one `geopath` entry per `direction_id`, each with a `paths` array of whitespace-separated `"lat, lng"` strings) **and** every stop's coordinates. The run-pattern map (issue #187) therefore fetches the pattern for the timeline, then makes a companion `stops/route` call and joins geopath + coordinates onto the run by `route_id` + `direction_id`.
