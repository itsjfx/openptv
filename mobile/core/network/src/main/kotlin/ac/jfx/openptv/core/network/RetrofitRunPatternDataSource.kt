 ...
 *
 * No `date_utc` is sent: the endpoint then defaults to "the pattern for this run today", which
 * includes the stops the service has already called at — exactly what issue #132 wants (past
 * stops dimmed, future stops live).
 */
        ...
        override suspend fun getRunPattern(
            ...
            val path =
                "pattern/run/${runRef.value}/route_type/$typeCode" +
                    "?expand=Stop&expand=Route&expand=Direction"
